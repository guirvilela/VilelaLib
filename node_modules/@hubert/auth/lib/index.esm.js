import { camelCaseProps, loadCache, loadCookies, parseJwt, purgeCache, removeCache, removeCookie, saveCache, saveCookie } from '@vilela/shared';
...
function AuthProvider(_ref) {
  ...
  function login(accessToken, erpToken, expires, callback) {
    ...
    saveCookie(_defineProperty({}, keys.accounts, accessToken), expires);
    if (keys.erp && erpToken) {
      setErpToken(erpToken);
      saveCookie(_defineProperty({}, keys.erp, erpToken), expires);
    }
  ...
  function logout(callback) {
    ...
    setUser(undefined);
    removeCookie(configs.logoutRemoveAllCookies ? configs.accessTokenKey : keys.accounts);
    if (configs.erpTokenKey && keys.erp) {
      removeCookie(configs.logoutRemoveAllCookies ? configs.erpTokenKey : keys.erp);
    }
    if (purge) {
      purgeCache();
    }
  ...
  React.useEffect(function () {
    var accessToken = loadCookies(configs.accessTokenKey).find(function (x) {
      return !!x;
    });
    setAccessToken(accessToken);
    if (accessToken) {
    ...
    if (configs.erpTokenKey) {
      var _erpToken = loadCookies(configs.erpTokenKey).find(function (x) {
        return !!x;
      });
...
function withAuth(config) {
  return function (Component) {
    return /*#__PURE__*/React.memo(function Auth(props) {
